...
c         and face grid doesn't align
        ...
        call hfmm3dpartstost(ier,iprec,zk,nsource,source,ifcharge,
     1          charge,ifdipole,dipstr,dipvec,ifpot,pot,
     2          iffld,fld,ntarg,targ,ifpottarg,pottarg,
     3          iffldtarg,fldtarg)


        m=min(nsource,100)

        call geterr(zk,nsource,source,ifcharge,charge,ifdipole,dipstr,
...
c-----------------------------------------------------------------
        ...
        subroutine geterr(zk,nsource,source,ifcharge,charge,ifdipole,
...
C
        rerr = 0
        if(ifpottarg.eq.1) call h3derror(pottarg,pot2,m,aerr,rerr)
     
        rgerr = 0
        if(iffldtarg.eq.1) call h3derror(fldtarg,fld2,3*m,aerr,rgerr)
