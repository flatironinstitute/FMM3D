...
template <class ValueType> void ConstVector<ValueType>::Init(Long dim_, Iterator<ValueType> data_, bool own_data_) {
  dim = dim_;
  capacity = dim;
  own_data = own_data_;
  if (own_data) {
...
template <class ValueType> ConstVector<ValueType>::ConstVector() {
  Init(0);
}

template <class ValueType> Vector<ValueType>::Vector() : ConstVector<ValueType>() {}


template <class ValueType> Vector<ValueType>::Vector(Long dim_, Iterator<ValueType> data_, bool own_data_) {
  this->Init(dim_, data_, own_data_);
}

template <class ValueType> Vector<ValueType>::Vector(const Vector<ValueType>& V) {
  this->Init(V.Dim(), (Iterator<ValueType>)V.begin());
}
...
template <class ValueType> void ConstVector<ValueType>::Swap(ConstVector<ValueType>& v1) {
  Long dim_ = dim;
...
template <class ValueType> void Vector<ValueType>::ReInit(Long dim_, Iterator<ValueType> data_, bool own_data_) {
  ...
  this->Swap(tmp);
#else
  if (own_data_ && this->own_data && dim_ <= this->capacity) {
    this->dim = dim_;
    if (data_ != NullIterator<ValueType>()) {
      memcopy(this->data_ptr, data_, this->dim);
    }
  } else {
    Vector<ValueType> tmp(dim_, data_, own_data_);
    this->Swap(tmp);
...
template <class ValueType> void ConstVector<ValueType>::Write(const char* fname) const {
...
template <class ValueType> void Vector<ValueType>::Read(const char* fname) {
  ...
  if (this->Dim() != (Long)(dim_[0] * dim_[1])) ReInit(dim_[0] * dim_[1]);
  if (dim_[0] && dim_[1]) readlen = fread(&begin()[0], sizeof(ValueType), dim_[0] * dim_[1], f1);
  assert(readlen == (Long)(dim_[0] * dim_[1]));
  SCTL_UNUSED(readlen);
  fclose(f1);
}

template <class ValueType> inline Long ConstVector<ValueType>::Dim() const { return dim; }
...
template <class ValueType> void Vector<ValueType>::SetZero() {
  if (this->Dim() > 0) memset<ValueType>(this->data_ptr, 0, this->Dim());
}

template <class ValueType> Iterator<ValueType> Vector<ValueType>::begin() { return this->data_ptr; }

template <class ValueType> ConstIterator<ValueType> ConstVector<ValueType>::begin() const { return this->data_ptr; }

template <class ValueType> Iterator<ValueType> Vector<ValueType>::end() { return this->data_ptr + this->dim; }

template <class ValueType> ConstIterator<ValueType> ConstVector<ValueType>::end() const { return this->data_ptr + this->dim; }

template <class ValueType> void Vector<ValueType>::PushBack(const ValueType& x) {
  if (this->capacity > this->Dim()) {
    this->data_ptr[this->Dim()] = x;
    this->dim++;
  } else {
    Vector<ValueType> v((Long)(this->capacity * 1.6) + 1); // TODO: This also calls the constructor. Instead allocate but use the in-place constructor.
    memcopy(v.data_ptr, this->begin(), this->Dim());
    v.dim = this->Dim();
    Swap(v);

    assert(this->capacity > this->Dim());
    this->data_ptr[this->Dim()] = x;
    this->dim++;
  }
}
...
template <class ValueType> inline ValueType& Vector<ValueType>::operator[](Long j) {
  ...
  return this->data_ptr[j];
}

template <class ValueType> inline const ValueType& ConstVector<ValueType>::operator[](Long j) const {
  assert(j >= 0 && j < dim);
  return data_ptr[j];
}
...
template <class ValueType> Vector<ValueType>& Vector<ValueType>::operator=(const std::vector<ValueType>& V) {
  if (this->dim != V.size()) ReInit(V.size());
  memcopy(this->data_ptr, Ptr2ConstItr<ValueType>(&V[0], V.size()), this->dim);
  return *this;
}

template <class ValueType> Vector<ValueType>& Vector<ValueType>::operator=(const Vector<ValueType>& V) {
  if (this != &V) {
    if (this->dim != V.dim) ReInit(V.dim);
    memcopy(this->data_ptr, V.data_ptr, this->dim);
  }
  return *this;
}

template <class ValueType> Vector<ValueType>& Vector<ValueType>::operator+=(const Vector<ValueType>& V) {
  SCTL_ASSERT(V.Dim() == this->dim);
  for (Long i = 0; i < this->dim; i++) this->data_ptr[i] += V[i];
  Profile::Add_FLOP(this->dim);
  return *this;
}
...
template <class ValueType> Vector<ValueType> ConstVector<ValueType>::operator+(const ConstVector<ValueType>& V) const {
  Vector<ValueType> Vr(dim);
  SCTL_ASSERT(V.Dim() == dim);
  for (Long i = 0; i < dim; i++) Vr.data_ptr[i] = data_ptr[i] + V[i];
...
template <class ValueType> Vector<ValueType>& Vector<ValueType>::operator=(ValueType s) {
  for (Long i = 0; i < this->dim; i++) this->data_ptr[i] = s;
  return *this;
}
...
template <class ValueType> Vector<ValueType> ConstVector<ValueType>::operator+(ValueType s) const {
  Vector<ValueType> Vr(dim);
  for (Long i = 0; i < dim; i++) Vr.data_ptr[i] = data_ptr[i] + s;
  Profile::Add_FLOP(dim);
  return Vr;
}
